...
grouped_ratio = df.groupby('เกรดโครงการ')[['%ทาวโฮม', '%บ้านแฝด', '%บ้านเดี่ยว2ชั้น']].mean()
...
if submitted:
    ...
    สัดส่วน = np.clip(pred[4:7], 0.05, 0.9)
    if เกรด in grouped_ratio.index:
        ...
        สัดส่วน = (np.array(สัดส่วน) * 0.2 + historic_ratio * 0.8)
    ...
    หลังรวม = ทาวโฮม + บ้านแฝด + บ้านเดี่ยว2ชั้น
