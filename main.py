...
df.columns = df.columns.str.strip().str.replace(" ", "").str.replace("(", "").str.replace(")", "").str.replace(".", "", regex=False)
...
df['%พื้นที่ขาย'] = df.get('พื้นที่จัดจำหน่ายตรม', 0) / df['พื้นที่โครงการตรม']
df['%พื้นที่สาธา'] = df.get('พื้นที่สาธารณะตรม', 0) / df['พื้นที่โครงการตรม']

# พื้นที่ถนน / สาธารณะ
if 'พื้นที่ถนนรวมตรม' in df.columns and 'พื้นที่สาธารณะตรม' in df.columns:
    ถนน_ต่อ_สาธา = df['พื้นที่ถนนรวมตรม'] / df['พื้นที่สาธารณะตรม'].replace(0, 1)
    เฉลี่ย_ถนน_ต่อ_สาธา = ถนน_ต่อ_สาธา[(ถนน_ต่อ_สาธา > 0.1) & (ถนน_ต่อ_สาธา < 2)].mean()
    df['%พื้นที่ถนน'] = df['%พื้นที่สาธา'] * เฉลี่ย_ถนน_ต่อ_สาธา
else:
    df['%พื้นที่ถนน'] = 0

# Model inputs
X_raw = df[['จังหวัด', 'เกรดโครงการ', 'พื้นที่โครงการตรม', 'รูปร่างที่ดิน']]
y_ratio = pd.DataFrame({
    'สัดส่วนพื้นที่สาธา': df['%พื้นที่สาธา'],
    'สัดส่วนพื้นที่จัดจำหน่าย': df['%พื้นที่ขาย'],
    'สัดส่วนพื้นที่สวน': df.get('พื้นที่สวน5ของพื้นที่จัดจำหน่าย', 0) / df['พื้นที่โครงการตรม'],
    'จำนวนหลังต่อไร่': df['จำนวนหลัง'] / (df['พื้นที่โครงการตรม'] / 1600),
...
if st.button("🔍 พยากรณ์ผลลัพธ์"):
    input_df = pd.DataFrame.from_dict({
        ...
        'พื้นที่โครงการตรม': [พื้นที่_input],
    ...
    # คำนวณจำนวนหลังแยกตามประเภท พร้อมเงื่อนไขเกรดโครงการ
    th_units = round(results['สัดส่วนทาวโฮม'] * จำนวน_หลัง)
    twin_units = round(results['สัดส่วนบ้านแฝด'] * จำนวน_หลัง)

    if เกรด_input.upper() in ['ESSENCE', 'GRANDESSENCE', 'ELEGANCE']:
        sd2_units = 0
        sd3_units = จำนวน_หลัง - th_units - twin_units
    else:
        sd3_units = 0
        sd2_units = จำนวน_หลัง - th_units - twin_units
