...
# ... ส่วน config และ model เหมือนเดิม ...
...
if submitted:
    ...
    with col1:
        st.metric("พื้นที่จัดจำหน่าย (ตรว)", f"{พท_ขาย:,.0f}")
        st.markdown(f"→ %Error: **{err_ขาย:.2f}%**")
        st.metric("พื้นที่สวน (ตรว)", f"{พท_สวน:,.0f}")
        st.markdown(f"→ %Error: **{err_สวน:.2f}%**")
    with col2:
        st.metric("พื้นที่สาธารณะ (ตรว)", f"{พท_สาธา:,.0f}")
        st.markdown(f"→ %Error: **{err_สาธา:.2f}%**")
        st.metric("พื้นที่ถนน (ตรว)", f"{พท_ถนน:,.0f}")
        st.markdown(f"→ %Error: **{err_ถนน:.2f}%**")
    with col3:
        st.metric("จำนวนแปลงรวม", f"{หลังรวม:,.0f}")
        st.markdown(f"→ %Error: **{err_แปลง:.2f}%**")
        st.metric("จำนวนซอย", f"{ซอย:,.0f}")
        st.markdown(f"→ %Error: **{err_ซอย:.2f}%**")
    ...
    st.markdown("### 📊 สรุป % ความคลาดเคลื่อน")
    st.markdown(f"""
    - พื้นที่จัดจำหน่าย: **{err_ขาย:.2f}%**
    - พื้นที่สวน: **{err_สวน:.2f}%**
    - พื้นที่สาธารณะ: **{err_สาธา:.2f}%**
    - พื้นที่ถนน: **{err_ถนน:.2f}%**
    - จำนวนแปลงรวม: **{err_แปลง:.2f}%**
    - จำนวนซอย: **{err_ซอย:.2f}%**
    """)
