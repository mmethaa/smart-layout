...
def optimize_lot_count(total_area, ratio, min_lot_size, max_lot_size, enforce_even=False):
    """คำนวณจำนวนแปลงโดยใช้สัดส่วน และพื้นที่ต่อแปลง (min/max)"""
...
df['%บ้านเดี่ยว'] = df['บ้านเดี่ยว2ชั้น'] / df['จำนวนหลัง'].replace(0, 1)  # ✅ เปลี่ยนตรงนี้
...
df['%บ้านเดี่ยว2ชั้น'] = df['บ้านเดี่ยว2ชั้น'] / df['จำนวนหลัง'].replace(0, 1)  # ใช้สำหรับคำนวณสัดส่วนชื่อใหม่
...
LOT_SIZES = {
    "ทาวน์โฮม": (20.08, 45.84),
    "บ้านแฝด": (45, 70),  # สมมุติจากข้อมูลเฉลี่ย
    "บ้านเดี่ยว": (50, 150),
}

# -------------------- FUNCTION --------------------
def optimize_lot_count(area_total, target_ratio, min_size, max_size, even=False):
    area_target = area_total * target_ratio
    best_count, best_avg_size, min_diff = 0, 0, float("inf")
    for lots in range(1, 2000):
        avg_size = area_target / lots
        if min_size <= avg_size <= max_size:
            actual_ratio = (lots * avg_size) / area_total
            diff = abs(actual_ratio - target_ratio)
            if diff < min_diff:
                if even and lots % 2 != 0:
                    continue
                best_count, best_avg_size, min_diff = lots, avg_size, diff
    return best_count, best_avg_size
...
if submitted:
    ...
    # คำนวณจำนวนแปลงแต่ละประเภทจากพื้นที่ขายจริง
    ทาวโฮม, _ = optimize_lot_count(พท_ขาย_twa, ratio[0], *LOT_SIZES["ทาวน์โฮม"])
    บ้านแฝด, _ = optimize_lot_count(พท_ขาย_twa, ratio[1], *LOT_SIZES["บ้านแฝด"], even=True)
    บ้านเดี่ยว, _ = optimize_lot_count(พท_ขาย_twa, ratio[2], *LOT_SIZES["บ้านเดี่ยว"])

    หลังรวม = ทาวโฮม + บ้านแฝด + บ้านเดี่ยว + อาคารพาณิชย์
