...
# ------------- HELPER ---------------
def format_err(value):
    return f"{value:.2f}%" if value is not None else "N/A"

def calculate_error_from_lookup(predicted, project_input):
    จังหวัด, เกรด, รูปร่าง, พื้นที่ = project_input
    match = df_grouped[(df_grouped['จังหวัด'] == จังหวัด) & 
                       (df_grouped['เกรดโครงการ'] == เกรด) & 
                       (df_grouped['รูปร่างที่ดิน'] == รูปร่าง)]
    if match.empty:
        return {k: None for k in ['err_ขาย', 'err_สาธา', 'err_สวน', 'err_ถนน', 'err_แปลง', 'err_ซอย']}
    ...
    แปลง_จริง = row['จำนวนหลัง']
    ซอย_จริง = row['จำนวนซอย']
    err = lambda p, t: abs(p - t) / t * 100 if t else None

    return {
        'err_ขาย': err(predicted['พื้นที่ขาย'], พท_ขาย_จริง),
        'err_สาธา': err(predicted['พื้นที่สาธารณะ'], พท_สาธา_จริง),
        'err_สวน': err(predicted['พื้นที่สวน'], พท_สวน_จริง),
        'err_ถนน': err(predicted['พื้นที่ถนน'], พท_ถนน_จริง),
        'err_แปลง': err(predicted['จำนวนหลัง'], แปลง_จริง),
        'err_ซอย': err(predicted['จำนวนซอย'], ซอย_จริง)
    }
...
df_grouped = df.groupby(['จังหวัด', 'เกรดโครงการ', 'รูปร่างที่ดิน']).mean(numeric_only=True).reset_index()
...
if submitted:
    ...
    err_dict = calculate_error_from_lookup(
        predicted={
            'พื้นที่ขาย': พท_ขาย,
            'พื้นที่สาธารณะ': พท_สาธา,
            'พื้นที่สวน': พท_สวน,
            'พื้นที่ถนน': พท_ถนน,
            'จำนวนหลัง': หลังรวม,
            'จำนวนซอย': ซอย
        },
        project_input=(จังหวัด, เกรด, รูปร่าง, พื้นที่)
    )
    ...
    with col1:
        st.metric("พื้นที่ขาย (ตรว.)", f"{พท_ขาย:,.0f}", help=f"Error: {format_err(err_dict['err_ขาย'])}")
        st.metric("พื้นที่สวน (ตรว.)", f"{พท_สวน:,.0f}", help=f"Error: {format_err(err_dict['err_สวน'])}")
    with col2:
        st.metric("พื้นที่สาธารณะ (ตรว.)", f"{พท_สาธา:,.0f}", help=f"Error: {format_err(err_dict['err_สาธา'])}")
        st.metric("พื้นที่ถนน (ตรว.)", f"{พท_ถนน:,.0f}", help=f"Error: {format_err(err_dict['err_ถนน'])}")
    with col3:
        st.metric("จำนวนแปลงรวม", f"{หลังรวม:,.0f}", help=f"Error: {format_err(err_dict['err_แปลง'])}")
        st.metric("จำนวนซอย", f"{ซอย:,.0f}", help=f"Error: {format_err(err_dict['err_ซอย'])}")
